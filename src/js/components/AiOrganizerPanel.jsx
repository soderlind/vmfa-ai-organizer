...
export function AiOrganizerPanel() {
	...
	const fetchStats = useCallback(async () => {
		try {
			...
			// Ignore fetch errors; status UI may still function.
		}
	}, []);
	 ...
	 */
	const fetchCachedResults = useCallback(async () => {
		try {
			const response = await apiFetch({
	...
	}, [status.status, status.dry_run, fetchCachedResults]);
	...
	const handleStartScan = async () => {
		try {
			...
			setNotice({
				...
				message: dryRun
					? __(
							'Preview scan started. Results will be shown when complete. You can leave this page and return later.',
							'vmfa-ai-organizer'
						)
					: __(
							'Scan started. Media files are being organized. You can leave this page and return later.',
							'vmfa-ai-organizer'
						),
			});
		} catch (err) {
